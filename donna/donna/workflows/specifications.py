...
- A deliverable/artifact record MUST name a concrete output that will exist after the work is complete (a file, module, package, binary, schema, configuration, test suite, generated report, published document, metric in the metrics storage, dashbord, etc.).
- Each deliverable/artifact MUST be derived from explicitly available inputs (developer request, prior sections of this plan). Donna MUST NOT invent new deliverables that introduce new scope.
- Each deliverable/artifact MUST be externally verifiable by inspecting the repository/workspace, produced files, or runtime outputs — not by reading explanatory prose.
- Each deliverable/artifact record MUST be phrased as an existence statement using normative language, e.g. "MUST include …", "MUST produce …", "MUST add …".
- Each deliverable/artifact record MUST be atomic: exactly one deliverable per record (no "and/or", no bundles). If multiple outputs are required, split into multiple records.
...
- If the deliverable is generated by running a command, the record SHOULD specify the command entrypoint (e.g., via a CLI (Command-Line Interface) command name) and the expected output location, without describing internal implementation steps.
- Deliverables MUST NOT be vague (e.g., "updated code", "better docs"). They MUST be concrete enough that a reviewer can confirm presence/absence.
- Deliverables MUST NOT restate goals, objectives, constraints, or acceptance criteria. They must list *outputs*, not outcomes or pass/fail checks.
- The section MUST NOT be empty.

Source files as artifacts:

- Explicitly add source files (paths) as deliverables/artifacts only if the task is specifically about creating or modifying those files (e.g., "MUST add docs/cli.md …").
- Do not add source files as deliverables/artifacts if they are unknown at planning time (i.e. we do not know which files will be changed/added). In such cases, focus on higher-level deliverables (e.g., "MUST add CLI documentation" instead of listing specific files).
