...
- `session:usage:work_scope` — a specification that describes the scope of work to be done on the session.
- `session:work:work_execution` — a workflow that describes the step-by-step plan of work to be done on the session.
...
The work scope specification has a standard name `session:usage:work_scope` and describes the work to be done in the context of the current session.
...
The work execution workflow has a standard name `session:work:work_execution` and describes the step-by-step plan of work to be done in the context of the current session.
...
- If workflow operation includes research/design work, there results MUST be represented as concrete artifacts or changes in the `session:usage:work_scope` and `session:work:work_execution` artifacts.
