...
def _patch_session_globals(mocker: object, tmp_path: pathlib.Path) -> None:
    mocker.patch("donna.workspaces.sessions.project_dir", return_value=tmp_path)
    mocker.patch("donna.workspaces.sessions.config", return_value=Config(session_dir=".session/donna"))


class TestDir:
    def test_dir__creates_session_directory(self, mocker: object, tmp_path: pathlib.Path) -> None:
...
class TestPath:
    def test_returns_configured_session_path_without_creating_it(self, mocker: object, tmp_path: pathlib.Path) -> None:
        _patch_session_globals(mocker, tmp_path)
...
class TestStatePath:
    def test_returns_state_file_path_under_session_dir(self, mocker: object, tmp_path: pathlib.Path) -> None:
        _patch_session_globals(mocker, tmp_path)
...
class TestEnsureDir:
    def test_ensure_dir__creates_session_directory(self, mocker: object, tmp_path: pathlib.Path) -> None:
        _patch_session_globals(mocker, tmp_path)
...
class TestReadState:
    def test_read_state__returns_none_for_missing_state_file(self, mocker: object, tmp_path: pathlib.Path) -> None:
        _patch_session_globals(mocker, tmp_path)

        assert sessions.read_state() is None

    def test_read_state__returns_state_bytes(self, mocker: object, tmp_path: pathlib.Path) -> None:
        _patch_session_globals(mocker, tmp_path)
...
class TestWriteState:
    def test_write_state__writes_state_bytes(self, mocker: object, tmp_path: pathlib.Path) -> None:
        _patch_session_globals(mocker, tmp_path)
        sessions.dir()
