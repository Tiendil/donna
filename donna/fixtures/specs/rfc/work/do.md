...
2. Run the chosen workflow.
3. Ensure you know the workflow id created in the previous step (default is `project:.donna:session:execute_rfc` if not specified).
...
1. Run the workflow created by the plan step (default: `project:.donna:session:execute_rfc`) and complete it.
