...
** Fix poetry run donna artifacts view 'donna.specifications.default_text_artifacts_behavior'
*** must show the whole artifact
** we need a base type for Kinds that are paired for sections.
*** artifact/section kind id should be full (with namespace)? the same for all other ids?
*** add default section kind for text sections
...
** relation between ArtifactSectionKindId and OperationKindId (cleanup types?)
