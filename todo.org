...
* TODO New data layout + specs as text
** .donna/donna.toml
*** paths
** new layout
** spec on a new layout
** move old donna spec into layout
** move donna spec from AGENTS.md to layout
** should donna allow first-found replacement of artifacts in the paths?
** introduce new id types
** artifact_source.py -> artifacts.py?
** we have a kind of conflict between world/namespace and world/session/namespace, do we?
*** git worktrees instead of world/session? https://github.com/max-sixty/worktrunk
** artifact kind currently has id and namespace, which are equal. We should do something about it.
** IMPORTANT: return helpfull errors when we fail to parse markdown
** artifact ids: specifications/xxx.md -> specification.xxx? To make artifacts format agnostic (for the future)
** refactor markdown parsing code into smaller pieces
