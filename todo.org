...
** Full paths to kinds
*** full artifact kind config for head section
*** full operation kind config for operation sections
*** donna.ops -> donna.operations?
*** where to place operations/kinds classes?
*** what with introspection?
*** do smth with hardcoded id generators like `artifact_kind_id` and `ops_section_id`
*** !filesystem list artifacts does not find .py (uses hardcoded .md)
*** !we do not need list modules?
...
*** This is weired "Non-Python resources should be searched under the same `python.import.path.<namespace>` as the markdown files (like in filesystem world)"
