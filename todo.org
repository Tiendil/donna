...
*** `World.update(...)` must allow updating artifact source code in Python world if the world is writable
*** `World.read_state(...)` must be allowed to return None if there is no state saved yet
*** `World.read_state(...)` should work in Python world if the world is session-based
*** `World.write_state(...)` should work in Python world if it is writable and session-based
