...
* TODO Architecture stabilization
** remove introspection clie
** artifacts list improvements
*** output only heads of artifacts
*** list recusively all matching artifacts
*** list all head with glob expressions over ids
*** tags <-- move into github issue?
**** tags as default attribute of an artifact
**** search by tags
**** listing all tags
**** specifications for tags
** Artifact view improvements
*** allow references to sections by full id -> display one specific section of artifact
** remove hardcpode from worlds and sources
*** filesystem world uses hardcoded extensions instead of list of sources
*** sync world implementations with source implementations — there are a lot of hardcode there.
*** remove _MARKDOWN_ROOTS
*** => introspection uses worlds to list available artifact/primitives
** each kind/primitive must has reference to specification section that explains it
*** cli command to list primitives?
*** cli command to show primitive specification?
** review
*** donna.world.worlds
*** donna.world.sources
*** reoganize donna.primitives submodules
** builtins (make an issue on github)
...
* TODO We should improve the way we work with cells. I.e. we generate different kinds of cells for different use-cases. That is not reflected in the code well enough.
