...
** introspection
*** Artifact listing is broken because there is no separation of artifact kinds
...
*** `artifacts list` output the whole artifacts bodies — we need only standardized short info records about each artifact
*** list_artifact should be recursive (search in subdirectories as well)
...
**** Do we need speciciations for tags?
