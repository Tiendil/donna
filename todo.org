...
*** donna artifacts validate <artifact id>
*** jinja2 on artifact load or on section processing?
*** render modes
*** cli render mode
*** validation render mode
*** remove resuls from the code
*** remove results from the artifacts
** refactor end_to_end_story_cycle without refactoring records
...
** When finishing an action request, check that the next operation is allowed for the current operation
** `donna artifacts` — make namespace argument optional or remove it from validate-all?
** move code.operations into submodules
** validation should check that the last operation is always `finish`
...
* Organize standard cell kinds into enum (but allow custom kinds as well)
