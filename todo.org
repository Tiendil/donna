...
* TODO Operations on lists (goals as the first candidate)
** WORKING Schema must be built with a list of items in mind
** WORKING Agent request instruction must
*** either describe the loop to process each item
*** or create a new task that organizes the loop
** WORKING record name should be allowed to be None in record kind spec
** WORKING goal record kind is registered
** WORKING goal kind/record need ids generation logic
