...
** review how changes work, maybe we can refactor them or remove
***  Unused change types suggest partial/incomplete change tracking: ChangeAddToQueue, ChangeTaskContext, ChangeAddCell exist but are never emitted. This also makes State.last_cells effectively dead (donna/donna/machine/changes.py, donna/donna/machine/state.py).
*** Core state mutation happens outside the change system: WorkUnit.run mutates self.state directly, and State.complete_action_request mutates queue and action_requests directly without Change objects (donna/donna/machine/tasks.py, donna/donna/machine/state.py). This breaks the “all state changes flow through Change” intent.
...
** Validation
*** should we call validation on every artifact update?
*** should we call validation on every artifact load?
** Render scheme of workflow somewhere?
*** using the donna to render schema in some specific format looks expensive
*** maybe we can ask agent to do it? By adding instructions into specifications
*** it can be an actual workflow
