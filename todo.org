...
** Artifacts edition via donna
*** specification of the artifact format
*** update donna_usage.md
*** do we want to validate artifact contents before update?
** refactor end_to_end_story_cycle into generating artifacts
** remove records functionality
** remove workflows submodule
...
** tune workding of workflows (what to write before goto)
...
** We may want, by default load/upload artifacts in a custom donna-managed directory, so, no filenames are needed for agents.
